...
?>
      <form action="index.php?act=updatehd" method="post" >
       <div class="title" ><h2 style="text-align: center;"> Chi tiết đơn hàng</h2></div>
  <div class="mb-3">
  ...
  <input name="billaddress" value="<?php if(isset($bill_address)&&($bill_address!="")) echo $bill_address;?>" disabled type="text" class="form-control" placeholder="Địa chỉ" aria-label="Username" aria-describedby="addon-wrapping">
  </div>
  <div class="mb-3">
  <label for="disabledTextInput" class="form-label">Số điện thoại</label>
  <input name="billtel" value="<?php if(isset($bill_tel)&&($bill_tel!="")) echo $bill_tel;?>" disabled type="text" class="form-control" placeholder="Số điện thoại" aria-label="Username" aria-describedby="addon-wrapping">
  </div>

  <div class="mb-3">
  <label for="disabledTextInput" class="form-label">Phương thức thanh toán</label>
  <input name="billtel" value="<?php echo get_pttt($bank_method);?>" disabled type="text" disabled class="form-control" placeholder="Số điện thoại" aria-label="Username" aria-describedby="addon-wrapping">
  </div>
  ...
  <input name="bankname" value="<?php if(isset($bank_name)&&($bank_name!=""))  echo $bank_name;?>" disabled type="text" disabled class="form-control" placeholder="Bank name" aria-label="Username" aria-describedby="addon-wrapping">
  </div>
  <div class="mb-3">
  <label for="disabledTextInput" class="form-label">Tên tài khoản</label>
  <input name="bankuser" value="<?php if(isset($bank_user)&&($bank_user!="")) echo $bank_user;?>"disabled type="text" disabled class="form-control" placeholder="Bank user" aria-label="Username" aria-describedby="addon-wrapping">
  </div>
  <div class="mb-3">
  <label for="disabledTextInput" class="form-label">Số tài khoản</label>
  <input name="banknumber" value="<?php if(isset($bank_number)&&($bank_number!="")) echo $bank_number;?>" disabled disabled type="text" class="form-control" placeholder="Bank number" aria-label="Username" aria-describedby="addon-wrapping">
  </div>
  <div class="mb-3">
...
<?php } if($bill_status!=4 && $bill_status!=5){ ?>
  <div class="mb-3">
